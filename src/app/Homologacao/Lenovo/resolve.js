...
async function Rodar() {
    ...
    //Pegando todos os erros da LenovoSales
    await SalesFunction.getSalesNFE(12623, "ERRO").then(data => {

        //console.log(data)
        Comentario = Comentario.concat(Payloads.AddData(dayjs().format('DD-MM-YY HH:mm').toString()))
        Comentario = Comentario.concat(Payloads.AddTexto("Resumo diario dos erros da Lenovo feito via NodeJs", true, "yellow"))
    ...
    const newSales = await SalesFunction.getSalesNFE(12623, "ERRO")
